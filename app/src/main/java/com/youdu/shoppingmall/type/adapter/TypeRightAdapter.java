...
import com.youdu.shoppingmall.utils.DensityUtil;
...
public class TypeRightAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == HOT) {
            return new HotViewHolder(mLayoutInflater.inflate(
                    R.layout.item_hot_right, null), mContext);
        } else {
            return new OrdinaryViewHolder(mLayoutInflater.inflate(
                    R.layout.item_ordinary_right, null), mContext);
        }
    }
    ...
    class HotViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(final List<TypeBean.ResultBean.HotProductListBean> hot_product_list) {
            for (int i = 0; i < hot_product_list.size(); i++) {
                ...
                LinearLayout.LayoutParams lineLp = new LinearLayout.LayoutParams(ViewGroup
                        .LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                final LinearLayout myLinear = new LinearLayout(mContext);
                lineLp.setMargins(DensityUtil.dip2px(mContext, 5), 0, DensityUtil.dip2px
                        (mContext, 5), DensityUtil.dip2px(mContext, 20));
                ...
                //设置图片
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(DensityUtil.dip2px
                        (mContext, 80), DensityUtil.dip2px(mContext, 80));
                ImageView imageView = new ImageView(mContext);
                //请求图片
                imageLoader.displayImage(imageView, Constants.BASE_URl_IMAGE +
                        hot_product_list.get(i).getFigure());
                //设置距离底部有10个dp
                ...
                //设置价格
                LinearLayout.LayoutParams textViewLp = new LinearLayout.LayoutParams(ViewGroup
                ...
                myLinear.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        mContext.startActivity(GoodsInfoActivity.actionView(mContext, goodsBean));
                    }
                });
            }
